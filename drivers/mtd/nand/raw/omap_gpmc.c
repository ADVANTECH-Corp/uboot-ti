...
#if defined(CONFIG_BCH) || defined(CONFIG_NAND_OMAP_ELM)
/**
...
#endif /* CONFIG_BCH || CONFIG_NAND_OMAP_ELM */
				    ...
				    bool force_8bit)
{
	...
	if (force_8bit) {
		omap_nand_read_buf(mtd, buf, len, true);
		return;
	}
	...
	if (head) {
		omap_nand_read_buf(mtd, buf, head, false);
		buf += head;
		len -= head;
	}
	...
	if (ret < 0) {
		...
		omap_nand_read_buf(mtd, buf, len, false);
	} else if (tail) {
		buf += len - tail;
		omap_nand_read_buf(mtd, buf, tail, false);
	}
}
				...
				uint8_t *buf, int oob_required, int page)
{
	...
	chip->read_buf(mtd, oob, ecctotal, false);
